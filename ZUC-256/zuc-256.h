/****************************************************
	>>>祖冲之-256密码算法实现
	>>>2020/7/29
	>>>author:GRL
*****************************************************/
#include <ctime>



typedef unsigned char bit8;
typedef unsigned int bit32;

///LBFS的16个31bits寄存器单元，32bit中最高位无效
bit32 LFSR_S0;
bit32 LFSR_S1;
bit32 LFSR_S2;
bit32 LFSR_S3;
bit32 LFSR_S4;
bit32 LFSR_S5;
bit32 LFSR_S6;
bit32 LFSR_S7;
bit32 LFSR_S8;
bit32 LFSR_S9;
bit32 LFSR_S10;
bit32 LFSR_S11;
bit32 LFSR_S12;
bit32 LFSR_S13;
bit32 LFSR_S14;
bit32 LFSR_S15;

///有限状态自动机FSM层存储单元R1、R2
bit32 F_R1;
bit32 F_R2;

///比特重组层
bit32 BRC_X0;
bit32 BRC_X1;
bit32 BRC_X2;
bit32 BRC_X3;

///S盒
bit8 S0[256] = {
	
}; 

bit8 S1[256] =  {
	 
};

///7bits填充常数di
bit8 EK_d[16] = { 
	/*0x44, 0x5e, 0x48, 0x54,
	0xda, 0x80, 0x80, 0x80,
	0x80, 0x80, 0x80, 0x80,
	0x80, 0xa4, 0x20, 0x60
	01000100, 01011110, 01001000, 01010100,
	11011010, 10000000, 10000000, 10000000,
	10000000, 10000000, 10000000, 10000000,
	10000000, 10100100, 00100000, 01100000 //后补0*/
	0x22, 0x2f, 0x24, 0x2a,
	0x6d, 0x40, 0x40, 0x40,
	0x40, 0x40, 0x40, 0x40,
	0x40, 0x52, 0x10, 0x30
	/*00100010, 00101111, 00100100, 00101010,
	01101101, 01000000, 01000000, 01000000,
	01000000, 01000000, 01000000, 01000000,
	01000000, 01010010, 00010000, 00110000 //前补0*/
};

/*>>F:模（2^31-1）加，c = a + b mod 2^31 - 1*/
bit32 AddM(bit32 a, bit32 b)
{
	bit32 c = a + b;
	return (c & 0x7FFFFFFF) + (c >> 31);
}

/*>>f: 循环左移k位实现乘2^k，最高位置0*/
bit32 MulByPow2(bit32 x, bit32 k)
{
	return (((x) << k) | ((x) >> (31 - k))) & 0x7FFFFFFF;
}

/*>>F: LFSR初始化模式*/
void LfsrInitMode(bit32 u) //u是FSM层非线性函数输出
{
	bit32 mul, v;
	/* v = (2^15*S15 + 2^17*S13 + 2^21*S10 + 2^20*S4 + (1+2^8*)*S0 )， 同zuc128*/
	v = LFSR_S0;
	mul = MulByPow2(LFSR_S0, 8); 
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S4, 20);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S10, 21);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S13, 17);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S15, 15);
	v = AddM(mul, v);

	if (v == 0)
	{
		v = 0x7FFFFFFF;
	}

	/* update the state */
	LFSR_S0 = LFSR_S1;
	LFSR_S1 = LFSR_S2;
	LFSR_S2 = LFSR_S3;
	LFSR_S3 = LFSR_S4;
	LFSR_S4 = LFSR_S5;
	LFSR_S5 = LFSR_S6;
	LFSR_S6 = LFSR_S7;
	LFSR_S7 = LFSR_S8;
	LFSR_S8 = LFSR_S9;
	LFSR_S9 = LFSR_S10;
	LFSR_S10 = LFSR_S11;
	LFSR_S11 = LFSR_S12;
	LFSR_S12 = LFSR_S13;
	LFSR_S13 = LFSR_S14;
	LFSR_S14 = LFSR_S15;
	LFSR_S15 = AddM(u, v); //【different】

	/* S15为0则令S15=2^31-1 */
	if (LFSR_S15 == 0)
	{
		LFSR_S15 = 0x7FFFFFFF;
	}
}

/*>>F: LFSR密钥流生成阶段*/
void LfsrWithWorkMode()
{
	bit32 mul, v;
	/* v = (2^15*S15 + 2^17*S13 + 2^21*S10 + 2^20*S4 + (1+2^8*)*S0 )， 同初始化阶段*/
	v = LFSR_S0;
	mul = MulByPow2(LFSR_S0, 8); 
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S4, 20);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S10, 21);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S13, 17);
	v = AddM(mul, v);

	mul = MulByPow2(LFSR_S15, 15);
	v = AddM(mul, v);

	/* update the state */
	LFSR_S0 = LFSR_S1;
	LFSR_S1 = LFSR_S2;
	LFSR_S2 = LFSR_S3;
	LFSR_S3 = LFSR_S4;
	LFSR_S4 = LFSR_S5;
	LFSR_S5 = LFSR_S6;
	LFSR_S6 = LFSR_S7;
	LFSR_S7 = LFSR_S8;
	LFSR_S8 = LFSR_S9;
	LFSR_S9 = LFSR_S10;
	LFSR_S10 = LFSR_S11;
	LFSR_S11 = LFSR_S12;
	LFSR_S12 = LFSR_S13;
	LFSR_S13 = LFSR_S14;
	LFSR_S14 = LFSR_S15;
	LFSR_S15 = v;

	/* S15为0则令S15=2^31-1 */
	if (LFSR_S15 == 0)	//【different】
	{
		LFSR_S15 = 0x7FFFFFFF;
	}
}
/*>>F: BR层bit重组， 同zuc128*/
void BitReorganization()
{
	BRC_X0 = ((LFSR_S15 & 0x7FFF8000) << 1) | (LFSR_S14 & 0xFFFF); // S15_H||S14_L， 舍弃S15最高位
	BRC_X1 = (LFSR_S11 << 16) | (LFSR_S9 >> 15); //S11_L||S9_H， 舍弃S9最高位
	BRC_X2 = (LFSR_S7 << 16) | (LFSR_S5 >> 15);  //S7_L||S5_H
	BRC_X3 = (LFSR_S2 << 16) | (LFSR_S0 >> 15);  //S2_L||S0_H*/
}

/*>>f: 循环左移k位*/
bit32 Rot(bit32 x, unsigned int k)
{
	return ((x) << k) | ((x) >> (32 - k));
}
/*>>f: L1， 同zuc128 */
bit32 L1(bit32 X)
{
	return (X ^ Rot(X, 2) ^ Rot(X, 10) ^ Rot(X, 18) ^ Rot(X, 24));
}

/*>>f: L2， 同zuc128 */
bit32 L2(bit32 X)
{
	return (X ^ Rot(X, 8) ^ Rot(X, 14) ^ Rot(X, 22) ^ Rot(X, 30));
}

/*>>f: s盒变换 */
bit32 Sbox(bit32 r) 
{
	bit32 a, b, c, d;
	a = S0[r >> 24];
	b = S1[(r >> 16) & 0xFF];
	c = S0[(r >> 8) & 0xFF];
	d = S1[r & 0xFF];
	return (a << 24) | (b << 16) | (c << 8) | d;
}

/*>>F: FSM层非线性函数F, */
bit32 F()
{
	bit32 W, W1, W2;
	W = (BRC_X0 ^ F_R1) + F_R2;
	W1 = F_R1 + BRC_X1;
	W2 = F_R2 ^ BRC_X2;
	bit32 r1, r2;
	r1 = L1((W1 << 16) | (W2 >> 16));	//(w1||w2)<<<16
	r2 = L2((W2 << 16) | (W1 >> 16));
	F_R1 = Sbox(r1);
	F_R2 = Sbox(r2);
	return W;
}

/*>>f: 将4个8bit拼接为31bit,b舍弃最高位， a||b||c||d */
bit32 Connect(bit8 a, bit8 b, bit8 c, bit8 d) 
{
	bit32 res = 0x00000000;
	res = (res | a) << 7;
	res = (res | b) << 8;
	res = (res | c) << 8;
	res = res | d;
	return res;
}

/*>>F: 初始化阶段*/
void Initialization(bit8* init_k,bit8* init_iv)
{
	//密钥、初始向量装载
	LFSR_S0 = Connect(init_k[0], EK_d[0], init_k[21], init_k[16]);
	LFSR_S1 = Connect(init_k[1], EK_d[1], init_k[22], init_k[17]);
	LFSR_S2 = Connect(init_k[2], EK_d[2], init_k[23], init_k[18]);
	LFSR_S3 = Connect(init_k[3], EK_d[3], init_k[24], init_k[19]);
	LFSR_S4 = Connect(init_k[4], EK_d[4], init_k[25], init_k[20]);
	LFSR_S5 = Connect(init_iv[0], EK_d[5]|init_iv[17], init_k[5], init_k[26]);
	LFSR_S6 = Connect(init_iv[1], EK_d[6]|init_iv[18], init_k[6], init_k[27]);
	LFSR_S7 = Connect(init_iv[10], EK_d[7]|init_iv[19], init_k[7], init_iv[2]);
	LFSR_S8 = Connect(init_k[8], EK_d[8]|init_iv[20], init_iv[3], init_iv[11]);
	LFSR_S9 = Connect(init_k[9], EK_d[9]|init_iv[21], init_iv[12], init_iv[4]);
	LFSR_S10 = Connect(init_iv[5], EK_d[10]|init_iv[22], init_k[10], init_k[28]);
	LFSR_S11 = Connect(init_k[11], EK_d[11]|init_iv[23], init_iv[6], init_iv[13]);
	LFSR_S12 = Connect(init_k[12], EK_d[12]|init_iv[24], init_iv[7], init_k[14]);
	LFSR_S13 = Connect(init_k[13], EK_d[13], init_iv[15], init_iv[8]);
	LFSR_S14 = Connect(init_k[14], EK_d[14]|(init_k[31] >>4), init_iv[16], init_iv[9]);
	LFSR_S15 = Connect(init_k[15], EK_d[15]|(init_k[31]&0x0f), init_k[30], init_k[29]);
	
	F_R1 = F_R2 = 0;
	//初始化阶段，共32+1轮
	for (int i=0; i<32; ++i){
		BitReorganization();
		bit32 u = F();
		LfsrInitMode(u >> 1);
	}
	BitReorganization();
	F(); 			// discard the output of F
	LfsrWithWorkMode();
}

/*>>F: ZUC密钥流生成 ,(密钥流存放数组，密钥流字长度)
void GenerateKeystream(bit32* pKeystream, int KeystreamLen)
{

	for (int i = 0; i < KeystreamLen; ++i)
	{
		BitReorganization();
		pKeystream[i] = F() ^ BRC_X3;
		LfsrWithWorkMode();
	}
}*/
#include <vector>;

int GenerateKeystream(bit32* pKeystream){
	time_t start_t = time(NULL);
	int i = 0;
	while(time(NULL) - start_t < 1){
		BitReorganization();
		pKeystream[0] = F() ^ BRC_X3;
		LfsrWithWorkMode();
		i++;
	}
	return i;
}











